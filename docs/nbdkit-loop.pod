...
which attaches a locally running nbdkit instance to the kernel device
F</dev/nbd0>.  I<-b 512> can be omitted when using
S<L<nbd-client(8)> E<ge> 3.19>.

 nbd-client -d /dev/nbd0

which detaches F</dev/nbd0>, and:

 nbd-client -c /dev/nbd0

which queries whether F</dev/nbd0> is attached or not.

The L<nbd-client(8)> command must be run as root.
