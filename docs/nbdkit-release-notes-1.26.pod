...
nbdkit 1.26.0 was released on B<XX XXXXXX 2021>.
...
L<nbdkit-streaming-plugin(1)> has been deprecated and will be removed
...
to choose that clients wait for an export to become free instead of
...
L<nbdkit-cow-filter(1)> now ignores flush/FUA requests which are
useful for a temporary overlay.  This filter now implements extents
...
depend on libselinux (Thomas Weißschuh).
...
nbdkit and plugins can now be built with C<-fvisibility=hidden>, which
...
When fuzzing nbdkit we now recommend using the C<-t 1> flag (which
