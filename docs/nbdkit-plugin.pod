...
Plugins have an ABI guarantee: a plugin compiled against an older
version of nbdkit will still work correctly when loaded with a newer
nbdkit.  We also try (but cannot guarantee) to support plugins
compiled against a newer version of nbdkit when loaded with an older
nbdkit, although the utility of such a plugin may be reduced where it
depended on features only provided by newer nbdkit.
...
newer nbdkit.  However, plugins written in other languages may have
to adapt to changes in the API when recompiled against a newer nbdkit.
