...
=head1 C<#include E<lt>nbdkit-plugin.hE<gt>>
...
to interact with stdin and stdout during C<.config> and
C<.config_complete>, and C<0> otherwise.  When nbdkit is started under
the single connection mode (C<-s>), the plugin must not directly
interact with stdin, because that would interfere with the client.
...
interactively (if C<nbdkit_stdio_safe> returns non-zero):
