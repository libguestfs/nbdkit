...
the system log (I<--log=syslog>).  For an explanation of what this
does and how the default is chosen, see L<nbdkit-service(1)/LOGGING>.
...
L<nbdkit-service(1)> — Run nbdkit as a service, systemd socket
activation.
