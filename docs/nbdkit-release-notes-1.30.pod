...
nbdkit 1.30.0 was released on B<XX Xxxxx 2022>.
...
New L<nbdkit-blocksize-policy-filter(1)> let you adjust or set the
...
L<nbdkit-blocksize-filter(1)>, L<nbdkit-cache-filter(1)> and
L<nbdkit-cow-filter(1)> adjust block size constraints from plugins.
...
L<nbdkit-memory-plugin(1)> (and others), using C<allocator=malloc,mlock=true>
was broken.  This was especially evident on non-Linux or on
Linux with 64K pages (eg. POWER).  It should now work correctly.
...
Multiple fixes to the Windows (MinGW) build.
