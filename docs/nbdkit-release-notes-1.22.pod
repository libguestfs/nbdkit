...
nbdkit 1.22.0 was released on B<XXX XXX 2020>.
...
L<nbdkit-streaming-plugin(1)> supports streaming to and from local
pipes.
...
L<nbdkit-ext2-plugin(1)> has been removed.  It was deprecated in
...
gzip-compressed data.  It replaces L<nbdkit-gzip-plugin(1)> which is
...
in the underlying plugin.  It replaces L<nbdkit-tar-plugin(1)> which
is deprecated and will be removed in S<nbdkit 1.26>.

New L<nbdkit-tls-fallback-filter(1)> provides alternate content when a
client connects without using TLS to a server that requires TLS
(Eric Blake).
...
L<nbdkit-python-python(3)> now supports parallel plugins.  These
