...
C<.config_complete>, C<.get_ready>, C<.after_fork>, C<.preconnect> and
...
filter which exports are advertised.  In S<nbdkit 1.22> plugins return
just a default export called C<"">.  In S<nbdkit 1.24> we plan to
allow plugins to control the list of exports and default export
advertised.
