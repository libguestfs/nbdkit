...
nbdkit-service - run nbdkit as a service, systemd socket activation
...
Using I<--log=stderr> forces messages to go to standard error.

=item Messages go to the system log (syslog):

When running from the command line, forked into the background.

Using I<--log=syslog> forces messages to go to the system log.
