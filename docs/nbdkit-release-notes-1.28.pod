...
nbdkit 1.28.0 was released on B<XXX September 2021>.

=head2 Security

CVE-2021-3716 "reset structured replies on starttls": nbdkit was
...
L<nbdkit-data-plugin(1)> has new leE<lt>NNE<gt>: and beE<lt>NNE<gt>:
prefixes for little and big endian words.  Also the plugin tries much
harder to optimize expressions.  The test suite has been greatly
expanded to catch potential regressions.

L<nbdkit-floppy-plugin(1)> now allows you to reserve free space (Nolan
Leake).

L<nbdkit-ssh-plugin(1)> no longer references SHA1 host keys, which are
insecure (Daniel Berrangé).
...
allocator=malloc no longer crashes because of memory corruption in
some corner cases (only seen on s390x, but could happen on any
architecture).  Meanwhile C<allocator=zstd> no longer crashes when
...
When running nbdkit from the source directory (using the wrapper) we
suppress C<$DEBUGINFOD_URLS> because of performance issues.
