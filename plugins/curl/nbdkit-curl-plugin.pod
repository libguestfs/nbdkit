...
To remove a header that curl has added, add the header without any
value:

 header='User-Agent:'

To add a custom header that has no value, you have to use semi-colon
instead of colon.  This adds an 'X-Empty:' header with no value:
