...
You can choose either MBR (limited to 4 partitions) or GPT (unlimited)
...
NBD clients may write to partitions, but will get an I/O error if they
try to change the virtual partition table or any padding areas of the
disk.
...
contain a boot sector or boot loader):
 ...
 nbdkit partitioning boot.img mbr-id=0x82 swap.img mbr-id=0x83 root.img
...
partition, followed by C<mbr-id=0x83> to set the MBR partition type
byte back to C<0x83> (Linux filesystem) for the remaining partition.
...
prefix to indicate this.  The default is C<0x83> which indicates a
Linux filesystem.

=item B<type-guid=>GUID

Set the GPT partition type GUID of all following files to C<GUID>.
The default is C<0FC63DAF-8483-4772-8E79-3D69D8477DE4> which indicates
a Linux filesystem.
...
assuming you have disk space for it), some clients will not be able to
handle this.
