...
You can choose either the MBR partition table type, which is limited
to 4 partitions, or the GPT partition table type which, in theory,
...
NBD clients may read and write freely to partitions.
...
Create a virtual disk containing boot, swap and root partitions:
 ...
 nbdkit partitioning \
            boot.img \
            mbr-id=0x82 swap.img mbr-id=0x83 \
            root.img
