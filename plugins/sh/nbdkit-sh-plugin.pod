...
C<$2> in all connected calls (eg. C<pread>, C<get_size>).  The
subdirectory may be deleted in C<close>, or you can rely on nbdkit
deleting the whole temporary directory when it exits.
