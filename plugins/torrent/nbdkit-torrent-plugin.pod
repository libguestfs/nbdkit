...
=item B<listen-interfaces=>LIST_OF_IP_ADDRESS:PORT

Listening ports that are opened for accepting incoming connections.
The parameter is a comma-separated list of IP address:port.

=item B<outgoing-interfaces=>LIST_OF_IP_ADDRESSES
