...
#
#   ./nbdkit -f -v python ./plugins/python/examples/url.py url=http://example.com/disk.img
#
...
def pread(h, buf, offset, flags):
    ...
    b = rp.read()
    if len(b) != count:
        raise RuntimeError("incorrect length of data returned: perhaps the server does not really support range requests")
    buf[:] = b
