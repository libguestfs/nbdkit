...
=head1 SYNOPSIS
 ...
 nbdkit ssh host=HOST [path=]PATH
            ...
            [config=CONFIG_FILE] [known-hosts=FILENAME]
...
In the C<config> and C<known-hosts> options, libssh expands some
C<%>-sequences.  Unfortunately there is no escape character for C<%>
itself, so there is no way to specify that character in the path.
