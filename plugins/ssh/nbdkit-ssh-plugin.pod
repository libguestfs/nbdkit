...
=head1 SYNOPSIS
 ...
 nbdkit ssh host=HOST [path=]PATH
            ...
            [verify-remote-host=false] [identity=FILENAME]
            [timeout=SECS]
...
expands some C<%>-sequences.  Unfortunately there is no escape
character for C<%> itself, so there is no way to specify that
