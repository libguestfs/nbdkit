...
In the C<config> option, libssh expands some C<%>-sequences.
Unfortunately there is no escape character for C<%> itself, so there
is no way to specify that character in the path.
