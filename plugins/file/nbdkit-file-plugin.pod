...
One of B<file> or B<dir> must be given to determine which mode
the server will use.
...
the client, where the client can request a list of available exports
using NBD_OPT_LIST.  A client that requests the default export (C<"">)
will be rejected.  However, you can use L<nbdkit-exportname-filter(1)>
to adjust what export names the client sees or uses as a default.  For
...
If present, this is the Windows version of the file plugin which lacks
certain features as noted in this manual.
